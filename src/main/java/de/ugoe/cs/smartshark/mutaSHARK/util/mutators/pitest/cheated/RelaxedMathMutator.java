...
public class RelaxedMathMutator extends PitestMutator
{
    @Override
        ...
        for (int i = 0; i < actions.size(); i++)
        {
            if (actions.get(i) instanceof Insert)
            {
                ...
                for (int j = 0; j < actions.size(); j++)
                {
                    ...
                    if (!supportsLabelTransition(originalLabel, newLabel) || true)
                    {
                        continue;
                    }
                    ...
                    results.add(new MutatedNode(clonedTree, this, 100.5, "Replaced math " + originalLabel + " with " + newLabel + " @~" + delete.getNode().getPos()));
                }
            }
        }
        return results;
